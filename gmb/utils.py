...
def is_drel_node(tup):
	if type(tup) == types.ListType and \
		len(tup) == 3 and \
		all([type(tu) == types.StringType and is_v(tu, "BK", "leastone") for tu in tup[1:]]):
		return True
	return False
...
#FOR CONDITION CARD NODE
##############################################
def is_cond_card_node(tup):
	if type(tup) == types.ListType and \
		tup[0] == "Card(":
		return True
	return False
def cond_card_wellform(tup):
	assert False, "no implementation"
##############################################

#FOR CONDITION TIME NODE
##############################################
def is_cond_time_node(tup):
	if type(tup) == types.ListType and \
		tup[0] in ["Txxx(", "Tyxx(", "Txmx(", "Txxd(", "Tymx(", "Tyxd(", "Txmd(", "Tymd("]:
		return True
	return False
def cond_card_wellform(tup):
	assert False, "no implementation"
