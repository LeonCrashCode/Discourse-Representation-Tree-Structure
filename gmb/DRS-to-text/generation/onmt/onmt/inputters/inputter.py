...
from onmt.utils.tree import Tree
import re
...
class StandardTreeField(LabelField):

    def __init__(self, **kwargs):
        kwargs['use_vocab'] = False
        kwargs['preprocessing'] = parse_standard_tree
        super(StandardTreeField, self).__init__(**kwargs)
...
def parse_standard_tree(bracket):

    tree, max_N = search_tree(bracket)
    edges = []
    edges.append(traversal_standard_edges(tree))
    edges.append(traversal_standard_edges_back(tree))
    return edges

def traversal_standard_edges(tree):
    neighbor_edges = []
    def traversal(l):
        if isinstance(l, list):
            ...
            while i < len(children):
                if isinstance(children[i], list):
                    ...
                    f = children[i]
                neighbor_edges.append([f["idx"], t["idx"]])
                i += 1

            for child in children:
                traversal(child)
    traversal(tree)
    return [[item[0] for item in neighbor_edges], [item[1] for item in neighbor_edges]]

def traversal_standard_edges_back(tree):
    neighbor_edges = []
    def traversal(l):
        if isinstance(l, list):
            f = l[0]
...
# def parse_tree(bracket):

#     bracket = bracket.strip().split()
...
#     return stack[0][0]
    ...
    #         t.idx = l[0]
    #         for c in l[1:]:
    #             t.children.append(traversal(c, l[0]))
    #     else:
    #         t.idx = l
    #     return t

    # tree = traversal(List, None)

    # return tree

# class GraphField(LabelField):

#     def __init__(self, **kwargs):
#         kwargs['use_vocab'] = False
#         kwargs['preprocessing'] = parse_edges
#         super(GraphField, self).__init__(**kwargs)

#     def process(self, batch, device=None):
#         return batch

# def search_tree(bracket):
...
#     for tok in bracket:
#         if tok[-1] == "(":
#             stack.append([{"idx":i, "name":tok}])
#             i += 1
#         elif tok == ")":
#             back = stack.pop()
#             stack[-1].append(back)
#         else:
#             stack[-1].append({"idx":i, "name":tok})
#             i += 1

#     List = stack[0][0]
#     return stack[0][0], i

# def traversal_childsum_nodes(tree):
#     childsum_nodes = []
#     def traversal(l):
#         if isinstance(l, list):
#             p = l[0]
#             children = l[1:]
#             if re.match("^DRS-[0-9]+\(", p["name"]) or p["name"] == "DRS(":
#                 childsum_nodes.append(p["idx"])
#             for child in children:
#                 traversal(child)
#     traversal(tree)
#     return childsum_nodes

# def traversal_childsum_edges(tree):
#     childsum_edges = []
#     def traversal(l):
#         if isinstance(l, list):
#             p = l[0]
...
#                     else:
#                         c = child
#                     childsum_edges.append([c["idx"], p["idx"]])
#             for child in children:
#                 traversal(child)
#     traversal(tree)
#     return [[item[0] for item in childsum_edges], [item[1] for item in childsum_edges]]
#     # return childsum_edges

# def traversal_neighbor_edges(tree, max_N):
#     neighbor_edges = []
#     def traversal(l):
#         if isinstance(l, list):
#             p = l[0]
...
#                 else:
#                     c = children[0]
#                 neighbor_edges.append([max_N, c["idx"]])
...
#                     if isinstance(children[i+1], list):
#                         t = children[i+1][0]
#                     else:
#                         t = children[i+1]
#                     neighbor_edges.append([f["idx"], t["idx"]])
...
#     neighbor_edges.append([max_N, 0])
#     traversal(tree)
#     return [[item[0] for item in neighbor_edges], [item[1] for item in neighbor_edges]]
#     # return neighbor_edges


# def traversal_parent_edges(tree):
#     parent_edges = []
#     def traversal(l):
#         if isinstance(l, list):
#             p = l[0]
...
#                 else:
#                     c = children[-1]
#                 parent_edges.append([c["idx"], p["idx"]])
#             for child in children:
#                 traversal(child)
#     traversal(tree)
#     return [[item[0] for item in parent_edges], [item[1] for item in parent_edges]]
#     # return parent_edges



# def traversal_childsum_edges_back(tree):
#     childsum_edges_back = []
#     def traversal(l):
#         if isinstance(l, list):
#             p = l[0]
...
#                     else:
#                         c = child
#                     childsum_edges_back.append([p["idx"], c["idx"]])
#             for child in children:
#                 traversal(child)
#     traversal(tree)
#     return [[item[0] for item in childsum_edges_back], [item[1] for item in childsum_edges_back]]
#     # return childsum_edges_back


# def traversal_neighbor_edges_back(tree, max_N):
#     neighbor_edges_back = []
#     def traversal(l):
#         if isinstance(l, list):
#             p = l[0]
...
#                     else:
#                         t = children[i-1]
#                     neighbor_edges_back.append([f["idx"], t["idx"]])
#                     i -= 1
#             for child in children:
#                 traversal(child)
#     traversal(tree)
#     return [[item[0] for item in neighbor_edges_back], [item[1] for item in neighbor_edges_back]]
#     # return neighbor_edges_back


# def traversal_parent_edges_back(tree):
#     parent_edges_back = []
#     def traversal(l):
#         if isinstance(l, list):
#             p = l[0]
...
#                 else:
#                     c = children[-1]
#                 parent_edges_back.append([p["idx"], c["idx"]])
#             for child in children:
#                 traversal(child)
#     traversal(tree)
#     return [[item[0] for item in parent_edges_back], [item[1] for item in parent_edges_back]]
#     # return parent_edges_back

# def parse_edges(bracket):
#     tree, max_N = search_tree(bracket)
#     # print(tree)
#     edges = []
#     edges.append(traversal_childsum_edges(tree))
#     edges.append(traversal_neighbor_edges(tree, max_N))
#     edges.append(traversal_parent_edges(tree))

#     edges.append(traversal_childsum_edges_back(tree))
#     edges.append(traversal_neighbor_edges_back(tree, max_N))
#     edges.append(traversal_parent_edges_back(tree))

#     edges.append(traversal_childsum_nodes(tree))
...
class NeighborTreeField(LabelField):

    def __init__(self, **kwargs):
        kwargs['use_vocab'] = False
        kwargs['preprocessing'] = parse_neighbor_edges
        super(NeighborTreeField, self).__init__(**kwargs)

    def process(self, batch, device=None):
        return batch

def parse_neighbor_edges(bracket):

    tree, max_N = search_tree(bracket)
    edges = []
    edges.append(traversal_neighbor_edges(tree))
    edges.append(traversal_parent_edges(tree))
    edges.append(traversal_neighbor_edges_back(tree, 0))
    edges.append(traversal_parent_edges_back(tree, 0))
...
def traversal_neighbor_edges(tree):
    neighbor_edges = []
    def traversal(l):
        if isinstance(l, list):
            p = l[0]
            ...
            if len(children) == 0:
                ...
                while i < len(children) - 1:
                    ...
                    if isinstance(children[i+1], list):
                        ...
                        t = children[i+1]
                    neighbor_edges.append([f["idx"], t["idx"], "n"])
                    i += 1
            for child in children:
                traversal(child)
    traversal(tree)
    return [[item[0] for item in neighbor_edges], [item[1] for item in neighbor_edges], [item[2] for item in neighbor_edges]]
    # return neighbor_edges
        ...
        if isinstance(l, list):
            ...
            if len(children) == 0:
                ...
                if isinstance(children[-1],list):
                    ...
                    c = children[-1]
                parent_edges.append([c["idx"], p["idx"], "p"])
            for child in children:
                traversal(child)
    traversal(tree)
    return [[item[0] for item in parent_edges], [item[1] for item in parent_edges], [item[2] for item in parent_edges]]
    # return parent_edges
            ...
            if len(children) == 0:
                ...
                while i > 0:
                    ...
                    if isinstance(children[i-1], list):
                        ...
                        t = children[i-1]
                    neighbor_edges_back.append([f["idx"]+max_N, t["idx"]+max_N, "bn"])
                    i -= 1
            for child in children:
                traversal(child)
    traversal(tree)
    return [[item[0] for item in neighbor_edges_back], [item[1] for item in neighbor_edges_back], [item[2] for item in neighbor_edges_back]]
    # return neighbor_edges_back

def traversal_parent_edges_back(tree, max_N):
    parent_edges_back = []
    def traversal(l):
        if isinstance(l, list):
            p = l[0]
            ...
            if len(children) == 0:
                ...
                if isinstance(children[-1],list):
                    ...
                    c = children[-1]
                parent_edges_back.append([p["idx"]+max_N, c["idx"]+max_N, "bp"])
            for child in children:
                traversal(child)
    traversal(tree)
    return [[item[0] for item in parent_edges_back], [item[1] for item in parent_edges_back], [item[2] for item in parent_edges_back]]
    # return parent_edges_back
...
def get_fields(
    ...
    assert src_data_type in ['text', 'img', 'audio', 'vec', 'tree'], \
        "Data type not implemented"
    assert not dynamic_dict or src_data_type == 'text' or src_data_type == 'tree', \
    ...
    if with_tree:
        if tree_type == "neighbor":
            fields["tree"] = NeighborTreeField()
        elif tree_type == "standard":
            fields["tree"] = StandardTreeField()
        else:
            assert False, "tree type unrecognized"
        # if with_tree_as_graph:
        #     fields["tree"] = GraphField()
        # else:
        #     fields["tree"] = TreeField()
