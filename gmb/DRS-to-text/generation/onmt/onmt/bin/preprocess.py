...
def process_one_shard(corpus_params, params):
    corpus_type, fields, src_reader, tgt_reader, align_reader, tree_reader, opt,\
         existing_fields, src_vocab, tgt_vocab = corpus_params
    i, (src_shard, tgt_shard, align_shard, tree_shared, maybe_id, filter_pred) = params
    ...
    tree_data = {"reader": tree_reader, "data": tree_shared, "dir": None}
    _readers, _data, _dir = inputters.Dataset.config(
        [('src', src_data), ('tgt', tgt_data), ('align', align_data), ('tree', tree_data)])
...
def build_save_dataset(corpus_type, fields, src_reader, tgt_reader,
    ...
    def shard_iterator(srcs, tgts, ids, aligns, trees, existing_shards,
                       existing_fields, corpus_type, opt):
        """
        Builds a single iterator yielding every shard of every corpus.
        """
        for src, tgt, maybe_id, maybe_align, tree in zip(srcs, tgts, ids, aligns, trees):
            ...
            tree_shareds = split_corpus(tree, opt.shard_size)
            for i, (ss, ts, a_s, t_s) in enumerate(
                    zip(src_shards, tgt_shards, align_shards, tree_shareds)):
                yield (i, (ss, ts, a_s, t_s, maybe_id, filter_pred))

    shard_iter = shard_iterator(srcs, tgts, ids, aligns, trees, existing_shards,
                                existing_fields, corpus_type, opt)

    with Pool(opt.num_threads) as p:
        dataset_params = (corpus_type, fields, src_reader, tgt_reader,
                          align_reader, tree_reader, opt, existing_fields,
                          src_vocab, tgt_vocab)
...
def preprocess(opt):
    ...
    build_save_dataset(
        'train', fields, src_reader, tgt_reader, align_reader, tree_reader, opt)

    if opt.valid_src and opt.valid_tgt:
        logger.info("Building & saving validation data...")
        build_save_dataset(
            'valid', fields, src_reader, tgt_reader, align_reader, tree_reader, opt)
