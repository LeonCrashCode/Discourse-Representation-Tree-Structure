...
class RNNDecoderBase(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 ...
                 reuse_copy_attn=False, copy_attn_type="general", no_init=False):
    ...
    def init_state(self, src, memory_bank, encoder_final):
        ...
        if self.no_init:
            self.init_state_from_scratch(src, memory_bank, encoder_final)
            return
    ...
    def init_state_from_scratch(self, src, memory_bank, encoder_final):

        batch_size = -1
        if isinstance(encoder_final, tuple):
            if isinstance(encoder_final[0],int):
                batch_size = encoder_final[0]
            else:
                batch_size = encoder_final[0].size(1)

            self.state["hidden"] = tuple(
                [torch.zeros(self.num_layers, batch_size, self.hidden_size).to(src.device),
                torch.zeros(self.num_layers, batch_size, self.hidden_size).to(src.device)]
            )
        else:
            if isinstance(encoder_final, int):
                batch_size = encoder_final
            else:
                batch_size = encoder_final.size(1)
            self.state["hidden"] = torch.zeros(self.num_layers, batch_size, self.hidden_size).to(src.device)
