...
def model_opts(parser):
    ...
    group.add('--tree_type', '-tree_type', type=str, default='childsum',
              choices=['n_ary', 'childsum', 'neighbor', 'mix'])
    group.add('--nary', '-nary', type=int, default=None)
    ...
    group.add('--no_init', '-no_init', action="store_true")
...
def preprocess_opts(parser):
    ...
    group.add('--train_tree', '-train_tree', nargs='+', default=[None],
              help="Path(s) to the training src-tgt alignment")
    ...
    group.add('--valid_tree', '-valid_tree', default=None,
              help="Path(s) to the validation src-tgt alignment")
    ...
    group.add('--with_tree', '-with_tree', action="store_true")
    group.add('--tree_type', '-tree_type', type=str, default="neighbor",
              choices=["neighbor", "standard"])
    group.add('--as_graph', '-as_tree', action="store_true")
...
def train_opts(parser):
    ...
    group.add('--input_limit', '-input_limit', default=-1, type=int)
...
def translate_opts(parser):
    ...
    group.add('--src', '-src', required=True,
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--tree', '-tree', default=None)
    group.add('--src_dir', '-src_dir', default="",
              help='Source directory for image or audio files')
    group.add('--tgt', '-tgt',
