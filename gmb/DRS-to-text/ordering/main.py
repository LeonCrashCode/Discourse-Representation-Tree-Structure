...
def parse_args():
    ...
    parser.add_argument('--writetrans', type=str, default=None, help='write translations for to a file')
    ...
    parser.add_argument('--test', type=str, help='test src file')
    ...
    parser.add_argument('--adagrad_accumulator_init', type=int, default=0)
    ...
    parser.add_argument('--learning_rate_decay', type=float, default=0.3)
    parser.add_argument('--start_decay_steps', type=int, default=0)
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        train_flag = True
        train_real = DocIter(train_data, args.batch_size, device="cuda" if args.gpu else "cpu",
                             train=train_flag,
                             shuffle=train_flag,
                             sort_key=lambda x: len(x.doc))

        devbatch = 1
        dev_real = DocIter(dev_data, devbatch, device="cuda" if args.gpu else "cpu", batch_size_fn=None,
