...
class SGRU(nn.Module):
    def __init__(self, s_emb, sh_dim, label_dim):
        ...
        self.s_gru = GRUCell(s_emb + sh_dim + label_dim + g_dim, sh_dim)

        self.g_gru = GRUCell(sh_dim, g_dim)
...
class GRNGOB(nn.Module):
    def __init__(self, s_emb, s_hidden, label_dim, dp=0.1, layer=2, edge_embed=None, edge_dp=0.1, gpu=False):
        super(GRNGOB, self).__init__()
        self.layer = layer
        self.dp = dp
        self.edgeemb = edge_embed
        self.edge_dp = edge_dp
        self.gpu = gpu

        self.slstm = SGRU(s_emb, s_hidden, label_dim)
        ...
        self.gate1 = nn.Linear(s_hidden + s_hidden + label_dim, s_hidden + label_dim)
    ...
    def forward(self, sent, smask, word, elocs):
        '''
        :param wmask: B E
        :param smask: B S
        :param word: B E H
        ...
        :return:
        '''
        # print("sent:", sent, sent.size())
        # print("smask:", smask, smask.size())
        # print("word:", word, word.size())
        # print("wmask:", wmask, wmask.size())
        # print("elocs:", elocs)
        batch = sent.size(0)
        snum = smask.size(1)
        # wnum = wmask.size(1)
        ...
        seq_edge = []
        for item in word.view(-1).tolist():
            if item == 1:
                continue
            seq_edge.append(item)
        # print("seq_edge:", seq_edge)
        r = 0
        matrix = sent.new_zeros(batch, snum, snum).long()
        for ib, eloc in enumerate(elocs):
            for ixw, loc in enumerate(eloc):

                matrix[ib, loc[0], loc[1]] = seq_edge[r]
                r+= 1
        assert r == len(seq_edge), "edge label not match"
        ...
        # print("label_emb", label_emb, label_emb.size())
        ...
        eye = torch.eye(snum).byte().cuda() if self.gpu else torch.eye(snum).byte()

        s2s = smask.new_ones(snum, snum).cuda() if self.gpu else smask.new_ones(snum, snum)
        ...
        # print("s2smatrix:",s2smatrix, s2smatrix.size())

        s_h = torch.zeros_like(sent).cuda() if self.gpu else torch.zeros_like(sent)
        # print("s_h:", s_h, s_h.size())
        g_h = sent.new_zeros(batch, self.s_hid) if self.gpu else sent.new_zeros(batch, self.s_hid)
        # print("g_h:", g_h, g_h.size())
        # e_h = sent.new_zeros(batch, wnum, self.e_hid)
        # print("e_h:", e_h, e_h.size())

        
        for i in range(self.layer):
            s_fr_h_expand = s_h.unsqueeze(2).expand(batch, snum, snum, self.s_hid)
            # print("s_fr_h_expand:", s_fr_h_expand, s_fr_h_expand.size())

            s_to_h_expand = s_h.unsqueeze(1).expand(batch, snum, snum, self.s_hid)
            # print("s_to_h_expand:", s_to_h_expand, s_to_h_expand.size())

            s_to_h_expand_edge = torch.cat((s_to_h_expand, label_emb), -1)

            s_s_l = torch.cat((s_fr_h_expand, s_to_h_expand_edge), -1)

            # print("s_s_l:", s_s_l, s_s_l.size())

            g = torch.sigmoid(self.gate1(s_s_l))

            s_neigh_s_h = s_to_h_expand_edge * g * s2smatrix.unsqueeze(3)

            s_neigh_s_h = s_neigh_s_h.sum(2)

            s_input = torch.cat((sent, s_neigh_s_h), -1)

            s_h, g_h = self.slstm(s_input, s_h, g_h, smask)
        ...
        #     print("s_neigh_s_h",s_neigh_s_h,s_neigh_s_h.size())
...
class PointerNet(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = GRNGOB(s_emb=args.d_rnn,
                              s_hidden=args.d_rnn,
                              label_dim=args.labeldim,
                              layer=args.gnnl, dp=args.gnndp,
                              edge_embed=self.src_embed, gpu=args.gpu)
    ...
    def forward(self, src_and_len, tgt_and_len, doc_num, ewords_and_len, elocs):
        # print("src_and_len:", src_and_len)
        document_matrix, _, hcn, key = self.encode(src_and_len, doc_num, ewords_and_len, elocs)

        # print("document_matrix",document_matrix, document_matrix.size())
        ...
        # print("pointed_mask",pointed_mask,pointed_mask.size())
        ...
        # print("pointed_mask_by_target", pointed_mask_by_target, pointed_mask_by_target.size())
        # print("target_mask", target_mask, target_mask.size())
    ...
    def encode(self, src_and_len, doc_num, ewords_and_len, elocs):
        ...
        # print("sentences:", sentences, sentences.size())
        # print("words_states:", words_states, words_states.size())
        ...
        para, hn = self.encoder(sentences, sents_mask, words, elocs)
...
def beam_search_pointer(args, model, src_and_len, doc_num, ewords_and_len, elocs):
    ...
    for t in range(target_t):
        ...
        if t == 0:
            ...
            pointed_mask[torch.arange(index.size(0)), index] = True
...
def train(args, train_iter, dev, fields, checkpoint):
    ...
    test_real = DocIter(test_data, 1, device="cuda" if args.gpu else "cpu", batch_size_fn=None,
    ...
    while True:
        for iters, batch in enumerate(train_iter):
            ...
            if steps % args.report == 0:
                ...
                print('epc:{} steps:{}/{} loss:{:.4f} t:{:.2f} lr:{:.8f}'.format(epc, steps, args.maximum_steps, total_loss, t2 - t1,
                                                                         opt.learning_rate()))
                total_loss = 0
                t1 = t2
            if steps % args.valid_steps == 0:
                ...
                with torch.no_grad():
                    ...
                    if args.loss:
                        ...
                        #                                                                       pmr, ktau))
                        print('Val acc:{:.4f} pmr:{:.2f} ktau:{:.4f}'.format(score, pmr, ktau))

                        checkpoint = {'model': model.state_dict(), 'args': args, 'loss': best_score}
                        torch.save(checkpoint, '{}/{}.pt'.format(args.model_path, steps))
