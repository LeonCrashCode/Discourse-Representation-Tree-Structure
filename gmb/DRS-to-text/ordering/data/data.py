...
class DocDataset(data.Dataset):
    def __init__(self, path, text_field, order_field, graph_field,
        ...
        with open(path, 'r', encoding=encoding) as f:
            graph = json.load(f)
            for inst in graph['data']:
                line = inst["nodes"].strip()
                lineeins = inst["edges"].strip()
                order = list(range(line.count('|||') + 1))
                if 'train' in path:
                    if len(order) > 1:
        ...
        while True:
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                for ex in minibatch:
                    ...
                    #     word_newlocs = []
                    #     # print(loc_role)
                    #     for lr in loc_role.split('|'): #0-2|1-1
                    #         oriloc, r = lr.split('-') #0-2
                    ...
                    if ex.entity.strip() != "":
                        ...
                        for edge in ex.entity.split(" ||| "):
                            fr, to, ty = edge.split()
                            elocs[-1].append([int(fr), int(to)])
                            ewords[-1].append(ty)

                    # print(elocs)
                    # print(ewords)
                # print("allsentences", allsentences)
                # print("elocs", elocs)
                # print("ewords", ewords)
