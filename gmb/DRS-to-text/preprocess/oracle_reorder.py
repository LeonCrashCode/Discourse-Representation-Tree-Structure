...
order_idx = 0
def assign_order(bracket, orders):
	
	def travel(root):
		global order_idx
		if re.match("^DRS-[0-9]+\($", root[0]):
			root.append(orders[order_idx])
			order_idx += 1
			assert len(root[-1]) == len(root)-3
			for child in root[:-1]:
			...
			for child in root:
				if type(child) == types.ListType:
					travel(child)
	travel(bracket)
def apply_order(bracket):

	def travel(root):
		if re.match("^DRS-[0-9]+\($", root[0]):
			order = root[-1]
			root = root[:-1]
			newroot = [root[0], root[1]]
			for i in order:
				newroot.append(root[2+i])
			root = newroot
		newroot = []
		for child in root:
			if type(child) == types.ListType:
				newroot.append(travel(child))
			else:
				newroot.append(child)
		return newroot

	return travel(bracket)
...
if __name__ == "__main__":
	...
	for line in open(sys.argv[2]):
		...
		assert len(line) == 2
		orders.append([int(i) for i in line[0].split()])
	...
	for inst in data["data"]:
		...
		assign_order(bracket, orders)
		bracket = apply_order(bracket)
