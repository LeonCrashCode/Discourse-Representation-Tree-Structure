...
def simplify_SDRS(bracket):
	...
	def travel(b):
		if is_sdrs_node(b):
			...
			mp = {}
			# print(b)
			# print(is_sdrs_node(b))
			for c in b[1:]:
				# print(c)
				if is_constituent_node(c):
					mp[c[0][:-1]] = str(idx)
					idx += 1
			# relative to K
			for c in b[1:]:
...
def unordered(bracket):

	newbracket = []

	if is_sdrs_node(bracket) or is_constituent_node(bracket):
		newbracket = bracket[0:1]
		for item in bracket[1:]:
			newbracket.append(unordered(item))
	elif is_drel_node(bracket):
		newbracket = bracket
	elif is_drs_node(bracket):
		newbracket = bracket[0:2]

		position = []
		stack_position = []
		i = 2
		for item in bracket[2:]:
			if is_unary_scope_node(item) or \
				is_binary_scope_node(item) or \
				is_drs_node(item) or \
				is_sdrs_node(item) or \
				is_prop_scope_node(item):
				position.append(-1)
				stack_position.append(i)
			else:
				position.append(i)
			i += 1
		permutation = list(np.random.permutation(position))

		for idx in permutation:
			if idx == -1:
				p = stack_position[0]
				stack_position = stack_position[1:]
				newbracket.append(unordered(bracket[p]))
			else:
				p = idx
				newbracket.append(bracket[p])
	elif is_unary_scope_node(bracket):
		newbracket = bracket[0:2]
		newbracket.append(unordered(bracket[2]))
	elif is_binary_scope_node(bracket):
		newbracket = bracket[0:2]
		newbracket.append(unordered(bracket[2]))
		newbracket.append(unordered(bracket[3]))
	elif is_prop_scope_node(bracket):
		newbracket = bracket[0:2]
		newbracket.append(unordered(bracket[2]))
	else:
